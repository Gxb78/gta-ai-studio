...
- **Le backend transporte le montage, il ne l'interprète pas** (décidé le 26 juillet 2026) : `project::Clip` et `project::Project` capturent tout champ inconnu dans un `#[serde(flatten)]`. Un champ ajouté côté TypeScript traverse la sauvegarde sans modification du Rust ; l'énumérer champ par champ garantissait au contraire une perte de données silencieuse au premier oubli.
